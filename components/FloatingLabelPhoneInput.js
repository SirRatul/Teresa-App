...
import { TextInput, Animated, View, Text, Dimensions } from 'react-native';
...
const FloatingLabelPhoneInput = (props) => {
  const [isFocused, setIsFocused] = useState(false)
  const isFirstRun = useRef(true)
  const animatedIsFocused = useState(new Animated.Value(props.value === '' ? 0 : 1))[0]

  const totalWidth = (Dimensions.get('window').width - RFPercentage(3)) * 0.9
  ...
  return <>
    ...
    <View style={{justifyContent: 'center', alignItems: 'center', width: '30%', borderRightWidth: 2, borderRightColor: Colors.spindle, padding: 12}}>
        <Text style={{color: Colors.danube}}>+88</Text>
    </View>
    <View style={{width: '70%', padding: 8, flexDirection: 'column'}}>
      <TextInput
        {...props}
        ref={props.myRef}
        style={[props.style]}
